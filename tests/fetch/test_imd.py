...
def test_download_frequency_15mins_sets_45min_branch(mock_report, monkeypatch, mock_ctx, mock_yaml_path, tmp_path):
    ...
    class TestCleanIMDData:
        def test_clean_imd_data_station_api(self):
            """Test cleaning of Station API data."""
            df = pd.DataFrame({
                "Date of Observation": ["2025-04-25"],
                "Time": ["13"],
            ...
            # Verify unwanted columns are dropped
            assert "WEATHER_ICON" not in cleaned_df.columns
            assert "WEATHER_MESSAGE" not in cleaned_df.columns
        ...
        def test_clean_imd_data_aws_arg(self):
            """Test cleaning of AWS ARG data."""
            df = pd.DataFrame({
                "DATE": ["2025-04-25"],
                "TIME": ["13:00:00"],
            ...
            cleaned_df = clean_imd_data(df, datacode="imd_AWS_ARG")
        ...
        def test_clean_imd_data_missing_columns(self):
            """Test cleaning with missing expected columns."""
            df = pd.DataFrame({
                "Station": ["DELHI"],
                "Date of Observation": ["2025-04-25"],
            ...
            # Verify missing columns are added with None values
            assert "Time" in cleaned_df.columns
            assert cleaned_df["Time"].isna().all()

        def test_clean_imd_data_invalid_data(self):
            """Test cleaning with invalid data values."""
            df = pd.DataFrame({
                "Date of Observation": ["invalid-date"],
                "Time": ["invalid-time"],
                "Mean Sea Level Pressure": ["NA"],
                "Wind Speed KMPH": [""],
                "Temperature": [32.5]
            })
            ...
            assert pd.isna(cleaned_df["meanSeaLevelPressure"].iloc[0])
            assert pd.isna(cleaned_df["windSpeed"].iloc[0])
            assert pd.isna(cleaned_df["timestamp"].iloc[0])

        def test_clean_imd_data_handles_empty_dataframe(self):
            """Test cleaning with an empty DataFrame."""
            df = pd.DataFrame()

            cleaned_df = clean_imd_data(df, datacode="imd_Station_API")

            # Verify the cleaned DataFrame is still empty
            assert cleaned_df.empty

        def test_clean_imd_data_adds_missing_expected_columns(self):
            """Test that missing expected columns are added."""
            df = pd.DataFrame({
                "Date of Observation": ["2024-05-05"],
                "Time": ["10"]
            })
            ...
            assert "stationName" in cleaned_df.columns
            assert pd.isna(cleaned_df["stationName"].iloc[0])

        def test_clean_imd_data_converts_na_and_blank_to_none(self):
            """Test that NA and blank values are converted to None."""
            df = pd.DataFrame({
                "Date of Observation": ["2024-05-05"],
                "Time": ["12"],
                "Temperature": ["NA"],
            ...
            # Verify NA and blank values are converted to None
            assert pd.isna(cleaned_df["temperature"].iloc[0])
            assert pd.isna(cleaned_df["humidity"].iloc[0])
